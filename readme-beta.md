...
- 新增“自动优选（当前地区）”，通过视频片段测速选择播放节点。
- 支持调整测速并发数（默认 1）和前两名精确复测。
- 新增测速状态、日志、节点缓存及“重新测速”。
- 支持播放过程中切换后续请求的节点，并在节点失败时回退原始源。
...
1. 在“视频 | 课堂 | 番剧”设置中选择地区，将节点设为 **自动优选（当前地区）**，点击“应用并刷新”。自动优选不用于直播或官方测速页。
2. 默认 **并发 1**，可选 1–8；默认开启前两名精确复测，可手动关闭。取得播放地址后开始测速，暂停时也会进行。
3. 节点结果缓存 10 分钟，再次使用时会采样验证；网络变化后可点“重新测速”，详情可查看“测速日志”。
4. 测速会下载视频片段，产生额外流量并可能影响首播。每轮请求预算为 8 MiB、8 秒，实际流量可能略高；时间有限时不一定测完全部候选。
5. 自动优选不改变清晰度、不绕过播放权限；没有可用结果时保留原始源。仍可手动选节点，或使用 B 站官方测速页面进行对照。
