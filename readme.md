...
1. 选择地区和 **自动优选（当前地区）**，点击“应用并刷新”。支持视频、课堂和番剧，不用于直播。
2. 默认单并发，并对前两名分段复测。缓冲充足时暂停自动测速，持续卡顿时可重测；也可点击“重新测速”并查看日志。
3. 测速会产生额外流量，已加入请求预算、跨标签页协调和冷却；遇到限流或连续拒绝时会暂停。手动重测也受这些限制。
4. 切源仅对后续请求生效，不改变清晰度或播放权限。浏览器不支持自动测速时，可手动选择节点。
